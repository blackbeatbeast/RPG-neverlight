...
const required = [
  ...
  'config/feature-flags.example.json',
  'docs/00_EXECUTIVE_BRIEF.md',
  'docs/01_HISTORICAL_REFERENCE.md',
  'docs/02_PRODUCT_VISION.md',
  'docs/03_GAME_DESIGN.md',
  'docs/04_UX_UI_SPEC.md',
  'docs/05_TECH_ARCHITECTURE.md',
  'docs/06_DATA_ECONOMY.md',
  'docs/07_CONTENT_R15_SAFETY.md',
  'docs/08_LEGAL_CLEAN_ROOM.md',
  'docs/09_MONETIZATION_READINESS.md',
  'docs/10_ROADMAP.md',
  'docs/11_CODEX_WORKPLAN.md',
  'docs/12_DECISIONS_OPEN_QUESTIONS.md',
  'docs/13_CONTENT_PIPELINE.md',
  'docs/CODEX_SKILLS_CATALOG.md',
  'docs/SOURCES.md',
  'docs/15_REQUIREMENTS_TRACEABILITY.md',
  'docs/16_FREE_TIER_BUDGET.md',
  'docs/17_CODEX_COMMANDS_JA.md',
  'provenance/ASSET_PROVENANCE.md',
  'provenance/RESEARCH_LOG.md',
  'content/examples/bundle.json',
  'content/first-region/bundle.json',
  'content/first-region/README.md',
  'content/first-region/character-sheets.md',
  'content/first-region/playtest-notes.md',
  'content/reviews/first-region-clean-room.md',
  'backlog/00_EPICS.md',
...
for (const key of [
  ...
  'ads.enabled',
  'supporterShop.enabled',
  'pvp.enabled',
  'market.enabled',
  'trade.enabled',
  'presentation.suggestive.enabled',
]) {
...
ok(
  /named_character_minimum_age:\s*20\b/.test(constraints),
  'Named character minimum age must be 20.',
);
ok(/monetization_enabled:\s*false\b/.test(constraints), 'Monetization must default false.');
ok(
  /paid_random_rewards_allowed:\s*false\b/.test(constraints),
  'Paid random rewards must be forbidden.',
);
...
const characters = [
  ...JSON.parse(read('content/examples/bundle.json')).characters,
  ...JSON.parse(read('content/first-region/bundle.json')).characters,
];
for (const character of characters) {
  ok(
    Number.isInteger(character.age) && character.age >= 20,
    `${character.id} must have integer age >= 20.`,
  );
  ok(Boolean(character.generalAssetId), `${character.id} needs a general asset.`);
  if (character.suggestiveAssetId) {
    ok(
      Boolean(character.suggestiveAssetId),
      `${character.id} has a suggestive asset reference without a general fallback.`,
    );
    ok(
      Boolean(character.generalAssetId),
      `${character.id} suggestive variant lacks general fallback.`,
    );
  }
}
...
const backlog = fs
  .readdirSync(path.join(root, 'backlog'))
...
for (const file of mdFiles) {
  ...
  if (
    !allowedHistorical.has(rel) &&
    fs.readFileSync(file, 'utf8').includes('ネバーワールドオンライン')
  ) {
...
console.log(`- Adult content characters checked: ${characters.length}`);
